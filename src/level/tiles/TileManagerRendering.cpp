...
void TileManager::DrawWallTile(int curr_tile_x, int curr_tile_y, Texture* tile_tex)
{
    ...
    if (left)
    {
        DrawTexturePro(*tile_tex, {10, 0, 1, 36}, Rectangle{
                           bbox_x,
                           bbox_y, 8, 72
                       }, {0, 0}, 0, WHITE);
    }
    ...
    if (up)
    {
        DrawTexturePro(*tile_tex, {0, 10, 36, 1}, Rectangle{
                           bbox_x,
                           bbox_y, 72, 8
                       }, {0, 0}, 0, WHITE);
    }
    if (down)
    {
        DrawTexturePro(*tile_tex, {0, 10, 36, 1}, Rectangle{
                           bbox_x,
                           bbox_y + TileSize - 8, 72, 8
                       }, {0, 0}, 0, WHITE);
    }

    DrawTexturePro(*tile_tex, rec, Rectangle{
                       bbox_x + (rec.x * 2),
                       bbox_y + (rec.y * 2), rec.width * 2, rec.height * 2
                   }, {0, 0}, 0, WHITE);
...
void TileManager::RenderForceFields(std::vector<Vector2> ForceFieldPos)
{
    ...
    for (Vector2 p : ForceFieldPos)
    {
        ...
        DrawTexturePro(game->GameResources->Textures["noise"],
                       {(p.x * TileSize / 2) + (float)(game->GetGameTime()), 0, rec.width, rec.height}, rec, {0, 0}, 0,
                       WHITE);
        DrawRectangleLinesEx(rec, 4.0f + sin(game->GetGameTime()) * 2.0f,
                             ColorLerp(GREEN, DARKGREEN, cos(game->GetGameTime()) / 3.0f));
    }
    EndBlendMode();
}
...
void TileManager::ProcessUniformLocations()
{
    if (DistortionUniformLocations.size() <= 0)
    {
        ...
        for (int i = 0; i < 100; i++)
        {
            int loc1 = GetShaderLocation(game->GameResources->Shaders["bounce_distort"],
                                         ("distortions[" + std::to_string(i) + "].position").c_str());
            int loc2 = GetShaderLocation(game->GameResources->Shaders["bounce_distort"],
                                         ("distortions[" + std::to_string(i) + "].strength").c_str());
            int loc3 = GetShaderLocation(game->GameResources->Shaders["bounce_distort"],
                                         ("distortions[" + std::to_string(i) + "].radius").c_str());
...
void TileManager::Update()
{
    UpdateDistance = Vector2{
        ...
        round(game->GameCamera->IntendedScreenHeight / TileSize) + 1
    };
    ...
    SetShaderValue(game->GameResources->Shaders["bounce_distort"], DistortionCountLocation, &DistortionCount,
                   SHADER_UNIFORM_INT);
...
void TileManager::DistortArea(Distortion DistortionForArea)
{
    ...
    {
        if (Vector2Distance(otherDis.Position, DistortionForArea.Position) < 25 && game->GetGameTime() - otherDis.
            SpawnTime <= FXLifetime / 2)
            return;
    }
...
void TileManager::ProcessDistortions()
{
    ...
    for (int i = 0; i < DistortionCount; i++)
    {
        ...
        SetShaderValue(game->GameResources->Shaders["bounce_distort"], PositionLocation, &SPosition,
                       SHADER_UNIFORM_VEC2);
        SetShaderValue(game->GameResources->Shaders["bounce_distort"], StrengthLocation, &Distortions[i].Strength,
                       SHADER_UNIFORM_FLOAT);
        SetShaderValue(game->GameResources->Shaders["bounce_distort"], RadiusLocation, &Distortions[i].Radius,
                       SHADER_UNIFORM_FLOAT);
    }
}