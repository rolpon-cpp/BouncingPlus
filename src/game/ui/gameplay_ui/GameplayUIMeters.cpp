...
void GameplayUI::DisplayPowerupMeter()
{
    ...
    {
        DrawRectangle(GetRenderWidth() - (125 + Margin) * 2, WeaponUITexture.texture.height - (100 + Margin), 125, 100,
...
{
    DrawRectangle(GetRenderWidth() - (125 + Margin), WeaponUITexture.texture.height - (100 + Margin), 125, 100,
                  ColorAlpha(BLACK, Alpha));
    float s2 = 70;
    ...
    if (game->MainPlayer->EnemyCombo > 1)
        DrawText(("+" + std::to_string(game->MainPlayer->EnemyCombo - 1)).c_str(),
                 ...
                 WeaponUITexture.texture.height - (100 + Margin) + 10 + s2 - (s2 / 2.5f),
...
void GameplayUI::DisplayCooldownMeter(float LowestX, float LowestY, float HighestWidth, float HighestHeight)
{
    if (game->MainPlayer != nullptr && game->MainPlayer->MainWeaponsSystem.CurrentWeapon != nullptr)
    {
        ...
        float percent = min(
            game->MainPlayer->MainWeaponsSystem.AttackCooldowns[game->MainPlayer->MainWeaponsSystem.CurrentWeaponIndex]
            / game->MainPlayer->MainWeaponsSystem.CurrentWeapon->Cooldown, 1.0f);
        Color g = WHITE;
        if (percent == 1.0f)
            g = GREEN;
        DrawRectangle(r.x + 10, r.y + 10 + ((r.height - 20) * (1 - percent)), r.width - 20, (r.height - 20) * percent,
                      g);

        //ammo meter
        if (game->MainPlayer->MainWeaponsSystem.CurrentWeapon->Ammo > 0)
        {
            DrawRectangle(r.x + r.width + Margin, WeaponUITexture.texture.height - (100 + Margin), 125, 100,
                          ColorAlpha(BLACK, Alpha));
            float s2 = 70;
            float s = MeasureText(
                to_string(
                    game->MainPlayer->MainWeaponsSystem.WeaponAmmo[game->MainPlayer->MainWeaponsSystem.
                                                                         CurrentWeaponIndex]).c_str(), s2);
            DrawText(
                to_string(
                    game->MainPlayer->MainWeaponsSystem.WeaponAmmo[game->MainPlayer->MainWeaponsSystem.
                                                                         CurrentWeaponIndex]).c_str(),
                (int)(r.x + r.width + Margin + 125 / 2 - s / 2),
                WeaponUITexture.texture.height - (100 + Margin) + 10, s2, WHITE);
...
void GameplayUI::DisplayHealthMeter()
{
    ...
    DrawRectangle((WeaponUITexture.texture.width / 2.0f - size / 2.0f) - Margin,
                  (WeaponUITexture.texture.height / 2.0f - FntSize / 2) - Margin, size + (Margin * 2),
                  FntSize + (Margin * 2),
                  ColorLerp(ColorLerp(ColorAlpha((game->MainPlayer->isInvincible ? WHITE : BLACK), Alpha), RED,
