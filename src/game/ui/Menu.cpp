...
void Menu::LevelSelect()
{
    ...
    Rectangle LevelSelectPanelRectangle = {
        (float)-GetRenderWidth() + (float)GetRenderWidth() / 2.0f - LevelSelectPanelSize.x / 2.0f,
        25 + (Offset2 / 1.5f), LevelSelectPanelSize.x, LevelSelectPanelSize.y
    };

    DrawRectangleRec({
                         LevelSelectPanelRectangle.x - CameraX, LevelSelectPanelRectangle.y,
                         LevelSelectPanelRectangle.width, LevelSelectPanelRectangle.height
                     }, ColorAlpha(BLACK, 0.5f));

    Rectangle play_bbox = {
        (GetRenderWidth() / 2.0f) - GetRenderWidth() - (int)(Shared->SharedUIAssets.ButtonImg.width / 2.0f),
        GetRenderHeight() - 106 + Offset1, 150, 56
    };
    ...
    DrawText("LEVEL SELECT",
             (int)(GetRenderWidth() / 2.0f) - (int)(MeasureText("LEVEL SELECT", 55) / 2.0f) - CameraX -
             GetRenderWidth(), LevelSelectPanelRectangle.y + 25, 55, WHITE);

    DrawLineEx(Vector2{LevelSelectPanelRectangle.x + 25.0f - CameraX, LevelSelectPanelRectangle.y + 100}, Vector2{
                   LevelSelectPanelRectangle.x + LevelSelectPanelSize.x - 25.0f - CameraX,
                   LevelSelectPanelRectangle.y + 100
               }, 4, WHITE);
    ...
    for (auto& [name, data] : Shared->LevelData)
    {
        ...
        Rectangle r = {
            LevelSelectPanelRectangle.x + 50 + (GetRenderWidth() - 900) + ow / 2 - rw / 2,
            (float)165 + (140 * i) + LevelSelectPanelRectangle.y, rw, rh
        };
        ...
        {
            c = GREEN;
            DrawRectangle(LevelSelectPanelRectangle.x + 25 - CameraX, 200 + LevelSelectPanelRectangle.y,
                          GetRenderWidth() - 900, GetRenderHeight() - 450, ColorAlpha(BLACK, 0.5f));
            ...
            if (!description.empty())
            {
                ...
                for (int i = 0; i < lines; i++)
                {
                    ...
                    while (MeasureText(description.substr(last_str_prog, str_prog - last_str_prog).c_str(), 25) <
                        alloc_size && str_prog < description.length())
                        str_prog++;
                    ...
                    DrawText(txt.c_str(), -(GetRenderWidth() - 145) - CameraX,
                             270 + (35 * i) + LevelSelectPanelRectangle.y, 25, WHITE);
                }
            }

            DrawText(("Difficulty level: " + to_string(data["difficulty"])).c_str(),
                     -(GetRenderWidth() - 145) - CameraX,
                     200 + GetRenderHeight() - 450 - 70 - 64 + LevelSelectPanelRectangle.y, 25, GetHealthColor(
                         1.0f - (data["difficulty"].get<float>() * 0.2f)
                     ));
            Rectangle play_bbox = {
                (float)-(GetRenderWidth() - 145), 200 + GetRenderHeight() - 450 - 70 + LevelSelectPanelRectangle.y, 150,
                56
            };
            if (Button({play_bbox.x - CameraX, play_bbox.y, play_bbox.width, play_bbox.height}, GetMousePosition(),
    ...
    Rectangle r = {
        LevelSelectPanelRectangle.x + 50 + (GetRenderWidth() - 900) + ow / 2 - rw / 2,
        (float)165 + 140 * (mi + 1) + LevelSelectPanelRectangle.y, rw, rh
    };
    DrawRectangleRec({r.x - CameraX, r.y, r.width, r.height}, ColorAlpha(BLACK, 0.5f));
    ...
    if (Button({r.x + r.width * 0.15f - 25 - CameraX, r.y + r.height / 2 - 25, 50, 50}, GetMousePosition(),
               Shared->SharedUIAssets.ButtonImg, Shared->SharedUIAssets.ButtonClick, "<"))
        CurrentLevelsPage -= 1;

    if (CurrentLevelsPage < 0)
        CurrentLevelsPage = (int)ceil(Shared->LevelData.size() / 3.0f) - 1;
    if (CurrentLevelsPage >= (int)ceil(Shared->LevelData.size() / 3.0f))
        CurrentLevelsPage = 0;
    std::string pageTxt = "PAGE " + to_string(CurrentLevelsPage + 1);
    DrawText(pageTxt.c_str(), r.x + r.width / 2 - MeasureText(pageTxt.c_str(), 50) / 2 - CameraX,
             r.y + r.height / 2 - 25, 50, WHITE);
}

void Menu::Credits()
{
    Rectangle CreditsPanelRectangle = Panel({
                                                GetRenderWidth() * 2 + GetRenderWidth() / 2.0f - ((float)
                                                    GetRenderWidth() - 200.0f) / 2 - CameraX,
                                                25, (float)GetRenderWidth() - 200.0f, (float)GetRenderHeight() - 180.0f
                                            }, "CREDITS", Offset1);

    Vector2 CreditsPanelSize = {CreditsPanelRectangle.width, CreditsPanelRectangle.height};

    DrawTexture(Shared->SharedUIAssets.RolponPFPImg, CreditsPanelRectangle.x + 25, CreditsPanelRectangle.y + 125.0f,
                WHITE);
    DrawTexture(Shared->SharedUIAssets.CozPFPImg, CreditsPanelRectangle.x + 25, CreditsPanelRectangle.y + 406.0f,
                WHITE);

    DrawTexture(Shared->SharedUIAssets.InkyPFPImg, CreditsPanelRectangle.x + 25 + (CreditsPanelSize.x - 50) / 2,
                CreditsPanelRectangle.y + 125.0f, WHITE);
    DrawTexture(Shared->SharedUIAssets.EggPFPImg, CreditsPanelRectangle.x + 25 + (CreditsPanelSize.x - 50) / 2,
                CreditsPanelRectangle.y + 406.0f, WHITE);

    float R = 127.0f + 127.0f * sin(GetTime());
    float G = 127.0f + 127.0f * cos(GetTime());
    float B = 127.0f + 127.0f * sin(GetTime() + 10);

    Vector2 RolponLoc = {CreditsPanelRectangle.x + 291, CreditsPanelRectangle.y + 125.0f};
    DrawText("Rolpon", RolponLoc.x, RolponLoc.y, 45.0f, Color{
    ...
    DrawText("inkyrblx", CreditsPanelRectangle.x + 291 + (CreditsPanelSize.x - 50) / 2,
             CreditsPanelRectangle.y + 125.0f, 45.0f, BROWN);

    DrawText("eggjsoto", CreditsPanelRectangle.x + 291 + (CreditsPanelSize.x - 50) / 2,
             CreditsPanelRectangle.y + 406.0f, 45.0f, ORANGE);
    ...
    // inky
    DrawText("Music & SFX Composer", CreditsPanelRectangle.x + 291 + (CreditsPanelSize.x - 50) / 2,
             CreditsPanelRectangle.y + 170.0f, 20, WHITE);

    // jay
    DrawText("Artist & Texture Maker", CreditsPanelRectangle.x + 291 + (CreditsPanelSize.x - 50) / 2,
             CreditsPanelRectangle.y + 406.0f + 45, 20, WHITE);

    float ThankYouTextWidth = MeasureText("Finally, thank YOU, for playing!", 40);
    DrawText("Finally, thank YOU, for playing!",
             CreditsPanelRectangle.x + CreditsPanelSize.x / 2 - ThankYouTextWidth / 2,
             CreditsPanelRectangle.y + CreditsPanelSize.y - 50, 40, WHITE);

    float FntSize = 24;
    float TxSize = MeasureText("Support me on YouTube!", FntSize);
    Rectangle r = {0, 0, TxSize + FntSize + 10, FntSize + 10};
    r.x = RolponLoc.x;
    r.y = RolponLoc.y + 100.0f;
    ...
    DrawTextureEx(Shared->SharedUIAssets.YTImg, Vector2{r.x + 12 + TxSize, r.y + 5}, 0, FntSize / 512.0f,
                  ColorAlpha(WHITE, 1.0f / TransparencyDiv));
}
...
void Menu::ShopStuff()
{
    ...
    Rectangle r = {0, 0, min(txtSize + 30.0f, 250.0f), 50.0f};
    r.x = GetRenderWidth() - r.width - 15.0f;
    r.y = GetRenderHeight() - r.height - 15.0f;

    r.y -= (Offset1 + Offset2) / 2.0f + Offset3;
...
void Menu::Update()
{
    ...
    DrawTexturePro(Shared->SharedUIAssets.MenuImg, {0, MenuImgOffsetY / 10.0f, 118.0f, 73.0f},
                   {
                       (float)GetRenderWidth() / 2.0f - CameraX / 10, (float)GetRenderHeight() / 2.0f,
                       (float)GetRenderWidth() * zoom, (float)GetRenderHeight() * zoom
                   },
    ...
    DrawTexture(Shared->SharedUIAssets.TitleImg,
    ...
    if (Button({
               ...
               Shared->SharedUIAssets.ButtonImg, Shared->SharedUIAssets.ButtonClick, "BACK") ||
        Button({
                   GetRenderWidth() + (GetRenderWidth() / 2.0f) - 75.0f - CameraX,
                   GetRenderHeight() - 106.0f + (Offset2 + Offset3) / 2, 150, 56
               }, GetMousePosition(),
               Shared->SharedUIAssets.ButtonImg, Shared->SharedUIAssets.ButtonClick, "BACK"))
        CameraTargetX = 0;
    ...
    if (Button({
                   ...
                   (float)SettingsButtonOffsetY + (Offset2 + Offset3) / 2, 150, 56
               }
               , GetMousePosition(), Shared->SharedUIAssets.ButtonImg, Shared->SharedUIAssets.ButtonClick, "SETTINGS"))
        CameraTargetX = 1;

    if (Button({
                   (GetRenderWidth() / 2.0f) - (int)(Shared->SharedUIAssets.ButtonImg.width / 2.0f) - CameraX,
                   (float)CreditsButtonOffsetY + Offset2, 150, 56
               },
...
{
    Reset();
    DetachAudioStreamProcessor(Shared->SharedUIAssets.MainMenuMusic.stream, AudioCallback);
}
