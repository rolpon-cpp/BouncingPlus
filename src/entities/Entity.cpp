    ...
    if (Vector2Distance({0, 0}, vel) != 0 && CollisionsEnabled)
    {
        ...
        for (int y = 0; y < 3; y++)
        {
            ...
            for (int x = 0; x < 3; x++)
            {
                ...
                if (
                    (game->GameTiles->TileTypes[tile_id] == WallTileType || (Type == EnemyType
                                                                                 ? game->GameTiles->TileTypes[tile_id]
                                                                                 == EnemyWallTileType
                                                                                 : false))
                    && CheckCollisionRecs(BoundingBox, bbox) && coord != LastVelBounceCoord)
                {
                    ...
                    float eRotation = std::atan2(VelocityMovement.y, VelocityMovement.x) * (180.0f /
                        3.141592653589793238463f);
        ...
        if (CollisionsEnabled)
        {
            ...
            for (int y = 0; y < 3; y++)
            {
                for (int x = 0; x < 3; x++)
                {
                    ...
                    if ((game->GameTiles->TileTypes[tile_id] == WallTileType || (Type == EnemyType
                        ? game->GameTiles->TileTypes[tile_id] == EnemyWallTileType
                        : false)))
                    {
                        Rectangle bbox = Rectangle{
                            bbox_x, bbox_y, game->GameTiles->TileSize, game->GameTiles->TileSize
                        };
