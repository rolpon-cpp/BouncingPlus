    ...
    if (game->GetGameTime() - CoverRefreshTimer >= 0.5f)
    {
        ...
        for (int i = 0; i < 60; i++)
        {
            ...
            if ((!d.HitAir) || Vector2Distance({plr_center_x, plr_center_y}, d.HitPosition) >= 500) // if we hit wall?
            {
                ...
                if (Vector2Distance(p.HitPosition, d.HitPosition) >= 150 && !p.HitAir && Vector2Distance(
                    {plr_center_x, plr_center_y}, d.HitPosition) >= 150)
                {
...
void WeaponBehavior::Update()
{
    ...
    if (Owner->MainWeaponsSystem.CurrentWeapon != nullptr && Owner->MainWeaponsSystem.TimeStartedReloading == -1 &&
        Owner->MainWeaponsSystem.CurrentWeapon->Ammo > 0 && Owner->MainWeaponsSystem.WeaponAmmo[Owner->MainWeaponsSystem
    ...
    if ((distance <= 800 && (distance <= 36 || FindPlayer())) || Owner->AngeredRangeBypassTimer > 0.0f)
    {
        if (distance >= 100 && !CoverSearching)
        {
            ...
            othermov.x += -(Target.x - center_x) / distance * Owner->Speed * (
                Owner->MainWeaponsSystem.CurrentWeapon != nullptr
                    ? (Owner->MainWeaponsSystem.CurrentWeapon->isMelee ? -1.0f : 1.0f)
                    : 1.0f);
            othermov.y += -(Target.y - center_y) / distance * Owner->Speed * (
                Owner->MainWeaponsSystem.CurrentWeapon != nullptr
                    ? (Owner->MainWeaponsSystem.CurrentWeapon->isMelee ? -1.0f : 1.0f)
                    : 1.0f);
        ...
        if (game->MainPlayer->Speed >= 200 && (Owner->MainWeaponsSystem.CurrentWeapon != nullptr
                                                   ? !Owner->MainWeaponsSystem.CurrentWeapon->isMelee
                                                   : true))
        {
            Target = Vector2Add(Target, Vector2Multiply(Vector2Normalize(game->MainPlayer->Movement),
                                                        {
                                                            game->MainPlayer->Speed * 0.2f,
                                                            game->MainPlayer->Speed * 0.2f
                                                        }));
        }
