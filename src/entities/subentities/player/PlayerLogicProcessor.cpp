...
void PlayerLogicProcessor::HandleFightMusic()
{
    ...
    std::string FightTrack = MyPlayer->game->GameShared->LevelData[MyPlayer->game->GameMode->GetCurrentLevelName()][
        "music"].get<std::string>() + "_layer" + to_string(ChosenLayer);
...
void PlayerLogicProcessor::AttackDashedEnemy(std::shared_ptr<Enemy> entity, bool already_attacked)
{
    ...
    Rectangle myRect = Rectangle{
        MyPlayer->BoundingBox.x - extra_size / 2, MyPlayer->BoundingBox.y - extra_size / 2,
        MyPlayer->BoundingBox.width + extra_size, MyPlayer->BoundingBox.height + extra_size
    };
    ...
    if (CheckCollisionRecs(myRect, entity->BoundingBox) && !already_attacked)
    {
        ...
        EnemyConcentration *= MyPlayer->game->GameShared->LevelData[MyPlayer->game->GameMode->GetCurrentLevelName()][
            "player"]["dash_concentration_boost"].get<float>();
        ...
        if (entity->Health <= 0)
        {
            ...
            MyPlayer->game->GameSounds->PlayGameSound("dash_hit_kill",
                                                      min(max(MyPlayer->VelocityPower / amount, 0.0f), 0.85f));
        }

        // dashing particle effect
        MyPlayer->game->GameParticles->ParticleEffect({
                                                          {
                                                              MyPlayer->BoundingBox.x + MyPlayer->BoundingBox.width / 2,
                                                              MyPlayer->BoundingBox.y + MyPlayer->BoundingBox.height / 2
                                                          },
                                                          1000,
                                                          PURPLE,
                                                          1100,
                                                          ...
                                                          1.3f,
                                                          PINK
                                                      }, 180 - Vector2LineAngle({0, 0}, MyPlayer->VelocityMovement) *
                                                      RAD2DEG, 30, 35);

        if (MyPlayer->game->GetGameTime() - LastFrozeGame >= 0.08f && (Damage >= 45.0f || entity->Health <= 0))
        {
            float StartFrames = 48.0f;

            StartFrames -= round(Damage / 10.0f);
            if (entity->Health <= 0)
                StartFrames -= 12.0f;

            StartFrames = max(StartFrames, 24.0f);
        ...
        MyPlayer->game->GameCamera->CameraPosition +=
            Vector2Normalize({(float)GetRandomValue(-35, 35), (float)GetRandomValue(-35, 35)}) * (MyPlayer->
                VelocityPower / 100);
...
void PlayerLogicProcessor::DashLogic()
{
    ...
    if (
        (MyPlayer->IsPreparingForDash && !MyPlayer->game->GameControls->IsControlDown("dash")) || MyPlayer->
        MainWeaponsSystem.ChargingProgress > 0.0f
    )
    {
        MyPlayer->IsPreparingForDash = false;
    }
    if (MyPlayer->game->MainPlayer->IsPreparingForDash || MyPlayer->game->MainPlayer->MainWeaponsSystem.
                                                                    TimeStartedReloading != -1)
    {
        ...
        if (MyPlayer->game->MainPlayer->IsPreparingForDash)
        {
            ...
            DrawTexturePro(anim_tex, Rectangle{0, 0, width, height * 6},
                           Rectangle{
                               MyPlayer->BoundingBox.x + MyPlayer->BoundingBox.width / 2 - cosf((FinalAngle) * DEG2RAD)
                               * 10,
                               MyPlayer->BoundingBox.y + MyPlayer->BoundingBox.height / 2 - sinf((FinalAngle) * DEG2RAD)
                               * 10,
                               width * size,
                               height * 6 * size
                           }, Vector2{width * size * 0.5f, height * 6 * size}, FinalAngle - 90,
                           ColorAlpha(WHITE, alpha));
            EndShaderMode();
        }
    }
    if ((IsMouseButtonDown(1) || IsMouseButtonDown(0)) && MyPlayer->IsPreparingForDash && MyPlayer->Health > 0 &&
        MyPlayer->game->GetGameTime() - DashTimeStart >= 0.35f)
    {
        DashCooldown = MyPlayer->game->GameShared->LevelData[MyPlayer->game->GameMode->GetCurrentLevelName()]["player"][
            "dash_base_cooldown"].get<float>() + (2.2f - min(
            static_cast<float>(MyPlayer->game->GetGameTime() - DashTimeStart), 1.1f) * 2);
        DashedEnemies.clear();
        MyPlayer->VelocityMovement = Vector2Subtract(
            GetScreenToWorld2D(GetMousePosition(), MyPlayer->game->GameCamera->RaylibCamera), {
                MyPlayer->BoundingBox.x + MyPlayer->BoundingBox.width / 2,
                MyPlayer->BoundingBox.y + MyPlayer->BoundingBox.height / 2
            });
        MyPlayer->VelocityPower = MyPlayer->game->GameShared->LevelData[MyPlayer->game->GameMode->GetCurrentLevelName()]
            ["player"]["dash_base_power"].get<float>()
        ...
        MyPlayer->VelocityPower *= MyPlayer->game->GameShared->LevelData[MyPlayer->game->GameMode->
            GetCurrentLevelName()]["player"]["dash_power_multiplier"].get<float>();
        MyPlayer->game->GameSounds->PlayGameSound("dash");
        MyPlayer->PlayerFrozenTimer = MyPlayer->game->GameShared->LevelData[MyPlayer->game->GameMode->
                GetCurrentLevelName()]["player"]["dash_frozen_multiplier"].get<float>() *
            min(max((MyPlayer->VelocityPower / MyPlayer->game->GameShared->LevelData[MyPlayer->game->GameMode->
                        GetCurrentLevelName()]["player"]["dash_base_power"].get<float>()), 0.35f), 1.1f);
        if (!MyPlayer->isInvincible)
        {
            MyPlayer->ToggleInvincibility();
            MyPlayer->InvincibilityResetTimer = MyPlayer->game->GameShared->LevelData[MyPlayer->game->GameMode->
                GetCurrentLevelName()]["player"]["dash_iframe_time"].get<float>();
        }
        if (IsMouseButtonDown(1))
        {
            ...
            MyPlayer->InvincibilityResetTimer = MyPlayer->game->GameShared->LevelData[MyPlayer->game->GameMode->
                GetCurrentLevelName()]["player"]["dodge_iframe_time"].get<float>() * 3.0f;
            DashCooldown = MyPlayer->game->GameShared->LevelData[MyPlayer->game->GameMode->GetCurrentLevelName()][
                "player"]["dodge_cooldown"].get<float>();
        }
        MyPlayer->IsPreparingForDash = false;
    ...
    if (MyPlayer->MainWeaponsSystem.CurrentWeapon != nullptr && MyPlayer->MainWeaponsSystem.CurrentWeapon->Ammo > 0)
    {
        ...
        DrawRectangle((int)(MyPlayer->BoundingBox.x + MyPlayer->BoundingBox.height + 20) + 5,
                      (int)(MyPlayer->BoundingBox.y + (MyPlayer->BoundingBox.height / 2) - (w / 2)) + 5,
                      h - 10,
                      ((float)MyPlayer->MainWeaponsSystem.WeaponAmmo[MyPlayer->MainWeaponsSystem.CurrentWeaponIndex] /
                          MyPlayer->MainWeaponsSystem.CurrentWeapon->Ammo) * (w - 10.0f),
