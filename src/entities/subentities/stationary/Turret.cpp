...
void Turret::SetTarget()
{
    Target = Vector2Add(GetCenter(), Vector2{
                            cos(TurretRotation * (2 * PI / 360)) * this->Range,
                            sin(TurretRotation * (2 * PI / 360)) * this->Range
                        });
}
...
bool Turret::PlayerIsVisible()
{
    ...
    return plrAngle >= targAngle - AngleRange / 2 && plrAngle <= targAngle + AngleRange / 2 && game->GameMiscTools->
        RayCast(GetCenter(), game->MainPlayer->GetCenter()) && Vector2Distance(
            GetCenter(), game->MainPlayer->GetCenter()) <= Range;
}
...
void Turret::Update()
{
    ...
    for (int i = 0; i < AngleRange / skip; i++)
    {
        ...
        if (Vector2Distance(GetCenter(), game->MainPlayer->GetCenter()) < Range + GetRenderWidth() / 2)
            DrawCircleSector(GetCenter(), Vector2Distance(GetCenter(), p.HitPosition), Angle - skip / 2,
                             Angle + skip / 2, skip, ColorAlpha(RED, 0.35f));
    ...
    case SearchingTurretState:
        {
            ...
            {
                float direction = (this->TurretRotation - this->LookingRotationGoal) / abs(
                    this->TurretRotation - this->LookingRotationGoal);
