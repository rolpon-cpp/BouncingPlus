...
bool Turret::PlayerIsVisible()
{
    ...
    return plrAngle >= targAngle - AngleRange/2 && plrAngle <= targAngle + AngleRange/2 && game->GameEngineTools.RayCast(GetCenter(), game->MainPlayer->GetCenter()) && Vector2Distance(GetCenter(), game->MainPlayer->GetCenter()) <= Range;
}
...
void Turret::Update()
{
    ...
    for (int i = 0; i < AngleRange / skip; i++)
    {
        ...
        auto p = game->GameEngineTools.RayCastPoint(GetCenter(),Vector2Add(GetCenter(), Vector2{X,Y}));
