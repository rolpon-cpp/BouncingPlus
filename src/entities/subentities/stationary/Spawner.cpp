...
Spawner::Spawner(Game& game, float MyX, float MyY) : Entity(game.GameResources->Textures["spawner"], {MyX, MyY, 36, 36},
                                                            0, game)
{
...
void Spawner::Render()
{
    ...
    BoundingBox.x = Lerp(BoundingBox.x, StartPos.x + RandPoint.x,
                         ((SpawnerIsActive > 0) ? 15 : 5) * game->GetGameDeltaTime());
    BoundingBox.y = Lerp(BoundingBox.y, StartPos.y + RandPoint.y,
                         ((SpawnerIsActive > 0) ? 15 : 5) * game->GetGameDeltaTime());
...
void Spawner::Update()
{
    ...
    if (SpawnerIsActive > 0 && game->GetGameTime() - SpawnTimer >= SpawnCooldown && game->GameTiles->EnemySpawnLocations
        .size() > 0)
    {
        for (int i = 0; i < GetRandomValue(1, 4); i++)
        {
            Vector2 p = game->GameTiles->EnemySpawnLocations[GetRandomValue(0,
                                                                            game->GameTiles->EnemySpawnLocations.size()
                                                                            - 1)];
            int times = 0;
            while (Vector2Distance(p, {game->MainPlayer->BoundingBox.x, game->MainPlayer->BoundingBox.y}) >= 3000 &&
                times < 10)
            {
                p = game->GameTiles->EnemySpawnLocations[GetRandomValue(
                    0, game->GameTiles->EnemySpawnLocations.size() - 1)];
                times++;
            }
            ...
            if (game->GameResources->EnemyWeaponNamesList.size() > 0)
            {
                ...
                std::vector<std::string> g = game->GameShared->LevelData[game->GameMode->GetCurrentLevelName()]["game"][
                    "banned_spawn_weapons"].get<std::vector<std::string>>();
